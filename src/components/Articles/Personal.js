...
const Personal = props => {
		...
		if (Array.isArray(value) && value.length == 0) {
			...
			setTutorial(prevState => {
				if (value.image != '') {
					...
					prevState.thumbnail = value.videoThumbnail;
					prevState.primaryImage = '';
				}
				return { ...prevState };
			});
	...
	const getVideoId = url => {
		...
		let idIdx = url.indexOf('?v=');
		if (idIdx != -1) {
			idIdx += 3;
			let latestString = url.substring(idIdx);
			let endIdx = latestString.indexOf('&');
			console.log({ latestString });
			if (endIdx != -1) {
				latestString = latestString.substring(0, endIdx);
			}
			return latestString;
		}
		return url;
	...
	useEffect(() => {
		...
		dispatch(fetchArticles({ section: 1 }));
	}, []);
									...
									<FilePicker
										...
										onSave={value => {
											setTutorial(prevState => {
												...
												prevState.thumbnail = `https://img.youtube.com/vi/${getVideoId(
													value
												)}/hqdefault.jpg`;
												return { ...prevState };
											});
