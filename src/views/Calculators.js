...
import {
	addCalculator,
	fetchCalculator,
} from '../store/actions/calculatorAction';
...
const Calculators = () => {
	...
	const handleCalculatorSubmit = async e => {
		...
		await dispatch(
			addCalculator(contents, _section, 0, activeCategory.categoryId)
		);
		setAddLoading(false);
	};
	...
	let { calculators } = useSelector(state => state.calculator);
	...
	useEffect(() => {
		if (!(calculators == 'idle' || calculators == 'loading')) {
			setContents(calculators);
		}
	}, [calculators]);
	return (
		<>
